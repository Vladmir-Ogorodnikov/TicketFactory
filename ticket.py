...
class TicketFactory:
    ...
    @staticmethod
    def create_ticket(user_id : UUID, row : int, number : int, age : int, vip_flag : bool) -> Ticket:
        if age < 12:
            return ChildTicket(user_id, row, number)
        elif vip_flag:
            return VIPTicket(user_id, row, number)
        return StandardTicket(user_id, row, number)

ticket = TicketFactory.create_ticket(uuid4(), 5, 11, 10, True)
print(isinstance(ticket, ChildTicket))